...
void CNifConvertDlg::OnBnClickedOk()
{
  ...
  //  get vertex color handling
  unsigned char vcHandling = (((CButton*) GetDlgItem(IDC_RADIO_VCREMOVE))->GetCheck() == BST_CHECKED) ? NCU_VC_REMOVE_FLAG : NCU_VC_ADD_COLORS;

  //  convert nif
  ncReturn = ncUtility.convertNif((CStringA(m_fileNameAry[0])).GetString(), (CStringA(m_fileNameAry[2])).GetString(), (CStringA(m_fileNameAry[1])).GetString(), (CStringA(m_texturePath)).GetString(), vcHandling);
  if (ncReturn != NCU_OK)
  {
